"""
This file contains evaluation methods that take in a
set of predicted labels and a set of ground truth labels
and calculate precision, recall, accuracy, f1, and metrics @k
"""
...
def print_metrics(metrics):
    if "auc_macro" in metrics.keys():
        ...
        logging.info("[MACRO] accuracy, precision, recall, f-measure")
        logging.info("%.4f, %.4f, %.4f, %.4f" % (metrics["acc_macro"], metrics["prec_macro"], metrics["rec_macro"], metrics["f1_macro"]))

    if "auc_micro" in metrics.keys():
        logging.info("[MICRO] accuracy, precision, recall, f-measure, AUC")
        logging.info("%.4f, %.4f, %.4f, %.4f, %.4f" % (metrics["acc_micro"], metrics["prec_micro"], metrics["rec_micro"], metrics["f1_micro"], metrics["auc_micro"]))
    else:
        logging.info("[MICRO] accuracy, precision, recall, f-measure")
        logging.info("%.4f, %.4f, %.4f, %.4f" % (metrics["acc_micro"], metrics["prec_micro"], metrics["rec_micro"], metrics["f1_micro"]))
...
if __name__ == "__main__":
    ...
    ind2c, _ = utils.load_full_codes(train_path, version=version)
